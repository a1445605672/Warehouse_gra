...
namespace DAL
{
	...
	public partial class storagelocation
	{
		...
		public bool Add(Model.storagelocation model)
		{
			...
			strSql.Append("sl_store_type_id,sl_store_min,sl_store_max,sl_store_area,sl_comment,sl_remain_bulk,sl_belong_chest)");
			strSql.Append(" values (");
			strSql.Append("@sl_store_type_id,@sl_store_min,@sl_store_max,@sl_store_area,@sl_comment,@sl_remain_bulk,@sl_belong_chest)");
			MySqlParameter[] parameters = {
					new MySqlParameter("@sl_store_type_id", MySqlDbType.Int32,32),
					...
					new MySqlParameter("@sl_belong_chest", MySqlDbType.Int32,32)};
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.storagelocation GetModel(int sl_id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select sl_id,sl_store_type_id,sl_store_min,sl_store_max,sl_store_area,sl_comment,sl_remain_bulk,sl_belong_chest from storagelocation ");
			strSql.Append(" where sl_id=@sl_id");
			MySqlParameter[] parameters = {
					new MySqlParameter("@sl_id", MySqlDbType.Int32)
			};
		...
		public Model.storagelocation DataRowToModel(DataRow row)
		{
			...
			if (row != null)
			{
				...
				if(row["sl_store_type_id"]!=null && row["sl_store_type_id"].ToString()!="")
				{
					model.sl_store_type_id=int.Parse(row["sl_store_type_id"].ToString());
				}
				...
				if(row["sl_belong_chest"]!=null && row["sl_belong_chest"].ToString()!="")
				{
					model.sl_belong_chest=int.Parse(row["sl_belong_chest"].ToString());
				}
			}
			return model;
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select sl_id,sl_store_type_id,sl_store_min,sl_store_max,sl_store_area,sl_comment,sl_remain_bulk,sl_belong_chest ");
