...
namespace DAL
{
	...
	public partial class sr_info
	{
		...
		public bool Add(Model.sr_info model)
		{
			...
			strSql.Append("sr_id,sr_name,sr_type,sr_resp_name,sr_resp_id,sr_contact_name,sr_contact_phone,sr_place,sr_create_time,sr_update_time)");
			...
			MySqlParameter[] parameters = {
					...
					new MySqlParameter("@sr_resp_id", MySqlDbType.Int32,11),
					new MySqlParameter("@sr_contact_name", MySqlDbType.VarChar,64),
					new MySqlParameter("@sr_contact_phone", MySqlDbType.Int32,16),
		...
		public bool Update(Model.sr_info model)
		{
			...
			parameters[9].Value = model.sr_id;
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.sr_info GetModel(int sr_id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select sr_id,sr_name,sr_type,sr_resp_name,sr_resp_id,sr_contact_name,sr_contact_phone,sr_place,sr_create_time,sr_update_time from sr_info ");
			strSql.Append(" where sr_id=@sr_id ");
			MySqlParameter[] parameters = {
					new MySqlParameter("@sr_id", MySqlDbType.Int32,32)			};
			parameters[0].Value = sr_id;
		...
		public Model.sr_info DataRowToModel(DataRow row)
		{
			...
			if (row != null)
			{
				if(row["sr_id"]!=null && row["sr_id"].ToString()!="")
				{
					model.sr_id=int.Parse(row["sr_id"].ToString());
				}
				if(row["sr_name"]!=null)
				{
					model.sr_name=row["sr_name"].ToString();
				}
				if(row["sr_type"]!=null && row["sr_type"].ToString()!="")
				{
				...
				if(row["sr_resp_id"]!=null && row["sr_resp_id"].ToString()!="")
				{
					model.sr_resp_id=int.Parse(row["sr_resp_id"].ToString());
				}
				if(row["sr_contact_name"]!=null)
				{
					model.sr_contact_name=row["sr_contact_name"].ToString();
				}
				if(row["sr_contact_phone"]!=null && row["sr_contact_phone"].ToString()!="")
				{
					model.sr_contact_phone=int.Parse(row["sr_contact_phone"].ToString());
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select sr_id,sr_name,sr_type,sr_resp_name,sr_resp_id,sr_contact_name,sr_contact_phone,sr_place,sr_create_time,sr_update_time ");
