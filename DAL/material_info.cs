...
namespace DAL
{
	...
	public partial class material_info
	{
		...
		public bool Add(Model.material_info model)
		{
			...
			strSql.Append("mat_id,mat_name,mat_father_id,mat_create_time,mat_fengdu,mat_state,mat_colour,mat_smell,mat_type,mat_type_id,mat_purpose,mat_source,mat_suanjianxing,mat_yuansuzhanbi,mat_yuansuliang)");
			strSql.Append(" values (");
			strSql.Append("@mat_id,@mat_name,@mat_father_id,@mat_create_time,@mat_fengdu,@mat_state,@mat_colour,@mat_smell,@mat_type,@mat_type_id,@mat_purpose,@mat_source,@mat_suanjianxing,@mat_yuansuzhanbi,@mat_yuansuliang)");
			MySqlParameter[] parameters = {
					new MySqlParameter("@mat_id", MySqlDbType.Int32,32),
					new MySqlParameter("@mat_name", MySqlDbType.VarChar,32),
					new MySqlParameter("@mat_father_id", MySqlDbType.Int32,11),
					new MySqlParameter("@mat_create_time", MySqlDbType.DateTime),
					new MySqlParameter("@mat_fengdu", MySqlDbType.Decimal,16),
					new MySqlParameter("@mat_state", MySqlDbType.Int32,10),
					new MySqlParameter("@mat_colour", MySqlDbType.Int32,10),
					new MySqlParameter("@mat_smell", MySqlDbType.Int32,10),
					new MySqlParameter("@mat_type", MySqlDbType.Int32,10),
					new MySqlParameter("@mat_type_id", MySqlDbType.Int32,32),
		...
		public bool Update(Model.material_info model)
		{
			...
			parameters[14].Value = model.mat_id;
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.material_info GetModel(int mat_id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select mat_id,mat_name,mat_father_id,mat_create_time,mat_fengdu,mat_state,mat_colour,mat_smell,mat_type,mat_type_id,mat_purpose,mat_source,mat_suanjianxing,mat_yuansuzhanbi,mat_yuansuliang from material_info ");
			strSql.Append(" where mat_id=@mat_id ");
			MySqlParameter[] parameters = {
					new MySqlParameter("@mat_id", MySqlDbType.Int32,32)			};
			parameters[0].Value = mat_id;
		...
		public Model.material_info DataRowToModel(DataRow row)
		{
			...
			if (row != null)
			{
				...
				if(row["mat_father_id"]!=null && row["mat_father_id"].ToString()!="")
				{
					model.mat_father_id=int.Parse(row["mat_father_id"].ToString());
				}
				...
				if(row["mat_state"]!=null && row["mat_state"].ToString()!="")
				{
					model.mat_state=int.Parse(row["mat_state"].ToString());
				}
				if(row["mat_colour"]!=null && row["mat_colour"].ToString()!="")
				{
					model.mat_colour=int.Parse(row["mat_colour"].ToString());
				}
				if(row["mat_smell"]!=null && row["mat_smell"].ToString()!="")
				{
					model.mat_smell=int.Parse(row["mat_smell"].ToString());
				}
				if(row["mat_type"]!=null && row["mat_type"].ToString()!="")
				{
					model.mat_type=int.Parse(row["mat_type"].ToString());
				}
				if(row["mat_type_id"]!=null && row["mat_type_id"].ToString()!="")
				{
					model.mat_type_id=int.Parse(row["mat_type_id"].ToString());
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select mat_id,mat_name,mat_father_id,mat_create_time,mat_fengdu,mat_state,mat_colour,mat_smell,mat_type,mat_type_id,mat_purpose,mat_source,mat_suanjianxing,mat_yuansuzhanbi,mat_yuansuliang ");
